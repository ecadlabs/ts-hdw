import { Ed25519, Hard, newSeedFromMnemonic } from "../src";
        ...
        keys: [
            {
                ...
                pub: "38739082097b0c14cb2cd92b800476e5d2310e30f921da58fc30d423a10de8a4",
            },
        ],
    },
    // BOLOS
...
describe("Ed25519", () => {
    for (const d of testData) {
        describe(d.seed || "mnemonic", () => {
            const seed = d.seed || newSeedFromMnemonic(d.mnemonic || "");
            ...
            for (const keyData of d.keys) {
                it(JSON.stringify(keyData.path.map((x) => x >>> 0)), () => {
                    ...
                    if (keyData.pub) {
                        expect(Buffer.from(key.pub()).toString("hex")).toBe(keyData.pub);
                    }
                });
            }
        });
